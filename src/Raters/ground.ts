import { Params } from "../types/rater";
...
const cm = (values: GroundRateInput) => {
  ...
  const lengthCostCM =
    length ** 2 / 8.5 + armorCosts[armor].CM + protectionCosts[protection].CM;
...
const el = (values: GroundRateInput) => {
  ...
  const lengthCostEL =
    3 *
    (length ** 2 / 85 + armorCosts[armor].EL + protectionCosts[protection].EL);
  ...
  const finalEL = shield
    ? systemCostEL *
      ...
      30
    : systemCostEL *
      (lengthCostEL + heavyCostEL + mediumCostEL + lightCostEL + rocketCostEL);
...
const cs = (values: GroundRateInput, costCM: number, costEL: number) => {
  ...
  return Math.ceil(
    Math.ceil((lengthCostCS + systemCostCS + 0.1 * (costCM + costEL)) * 20) /
      100
  );
};
